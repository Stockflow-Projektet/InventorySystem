...
namespace Inventory.Tests.Frontend.Services.Implementations
{
    public class ProductServiceTests
    {
        ...
        public async Task GetProductsAsync_ReturnsProducts()
        {
            ...
            {
                new ProductViewModel { ProductId = 1, Name = "Test Product 1" },
                new ProductViewModel { ProductId = 2, Name = "Test Product 2" }
            };

            mockHttp.When("http://test/api/products")
                    .Respond("application/json", System.Text.Json.JsonSerializer.Serialize(expectedProducts));
        ...
        public async Task GetProductByIdAsync_ReturnsSingleProduct()
        {
            ...
            mockHttp.When("http://test/api/products/42")
                    .Respond("application/json", System.Text.Json.JsonSerializer.Serialize(product));
